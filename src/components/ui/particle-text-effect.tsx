...
const DEFAULT_WORDS = ["OPENASSETS"]
...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Generate new color - Gray shades and light purple
    const colorChoice = Math.random()
    let newColor

    if (colorChoice < 0.5) {
      // Gray shades (lighter grays)
      const grayValue = Math.floor(Math.random() * 100) + 155 // 155-255 range
      newColor = {
        r: grayValue,
        g: grayValue,
        b: grayValue,
      ...
      if (purpleVariation < 0.33) {
        // Lavender
        newColor = { r: 200, g: 180, b: 230 }
      } else if (purpleVariation < 0.66) {
        // Light purple
        newColor = { r: 180, g: 160, b: 220 }
      } else {
        // Soft purple
        newColor = { r: 160, g: 140, b: 210 }
      }
    }
  ...
  const animate = () => {
    ...
    // Auto-advance words (only if more than one word)
    if (words.length > 1) {
  ...
  return (
    <div className={className}>
      ...
      />
      {showControls && (
        <div className="mt-4 text-white text-sm text-center max-w-md mx-auto">
          <p className="mb-2">Particle Text Effect</p>
          <p className="text-gray-400 text-xs">
            Right-click and hold while moving mouse to destroy particles
            {words.length > 1 && " • Words change automatically every 4 seconds"}
          </p>
        </div>
      )}
    </div>
