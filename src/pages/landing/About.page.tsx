...
export default function AboutPage() {
  ...
  useEffect(() => {
    if (organization && organization.repositories.length > 0 && contributors.length === 0) {
      const firstRepo = organization.repositories[0].name;
      fetchUiMetrics(firstRepo).catch(console.error);
    }
  }, [organization, contributors.length, fetchUiMetrics]);

  // Define clip paths for different image shapes
  const clipPaths = [
    "url(#clip-squiggle)",
    "url(#differentone16)",
    "url(#differentone8)",
  ...
  // Define rotation classes and animation variants
  const rotations = ["rotate-6", "-rotate-6", "-rotate-6", "rotate-6"];
  // const animationVariants = ["revealVariants", "revealVariants2", "revealVariants2", "revealVariants2"];
    ...
    >
      ...
      <div className="relative z-10 max-w-2xl mx-auto mt-10">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-gray-900 mb-6">
          ...
          >
            {
              "A Journey of a trillion bytes begins with a single bit !"
            }
          </VerticalCutReveal>
        </h2>
        ...
        >
          From day one, our mission has been to build high-impact Web3 infrastructure focused on real-world adoption. With experience spanning leading AI research labs, major Web3 ecosystems such as Binance and Stellar, and the successful launch of a DeFi startup, our team is focused on redefining how real-world assets are tokenized, starting with the Asian corridor and scaling globally.
        </TimelineContent>
      </div>
          ...
          contributors.map((contributor, index) => {
            ...
            return (
              <TimelineContent
                ...
                className="w-full h-full rounded-lg overflow-hidden cursor-pointer transition-transform hover:scale-105"
                style={{ clipPath: clipPaths[index % clipPaths.length] }}
                onClick={() => window.open(contributor.profileUrl, '_blank', 'noopener,noreferrer')}
              >
                <img
                  src={contributor.avatarUrl}
                  alt={contributor.name || contributor.author}
                  width={128}
                  height={160}
                  className={`object-cover w-full h-full ${rotations[index % rotations.length]}`}
                />
              </TimelineContent>
            );
          })
